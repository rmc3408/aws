...
async function processEachRecord(record: S3EventRecord) {
  ...
  try {
    ...
    if (transactionStatus === InvoiceTransactionStatusEnum.GENERATED) {
      await Promise.all(  //to do both at same time, since they are not dependent
        [
          webSocketServices.sendInvoiceStatus(connectionId, transactionId, InvoiceTransactionStatusEnum.RECEIVED),
          transactionRepo.updateStatusTransaction(transactionId, InvoiceTransactionStatusEnum.RECEIVED)
        ]
      )
    } else {
